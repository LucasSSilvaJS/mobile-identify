...
export default function EditarOdontogramaScreen({ navigation, route }) {
  ...
  const { carregarOdontogramaPorId, atualizarOdontograma } = useOdontogramas();
  ...
  useEffect(() => {
    carregarOdontograma();
  }, [odontogramaId, odontogramaParam]);

  const carregarOdontograma = async () => {
    try {
      setInitialLoading(true);
      
      let odontogramaData;
      if (odontogramaParam) {
        odontogramaData = odontogramaParam;
      } else if (odontogramaId) {
        odontogramaData = await carregarOdontogramaPorId(odontogramaId);
      } else {
        throw new Error('ID do odontograma não fornecido');
      }
      
      setIdentificacao(odontogramaData.identificacao?.toString() || '');
      setObservacao(odontogramaData.observacao || '');
    } catch (err) {
      console.error('Erro ao carregar odontograma:', err);
      Alert.alert(
        'Erro',
        'Erro ao carregar dados do odontograma. Tente novamente.',
        [
          {
            text: 'OK',
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      await atualizarOdontograma(odontogramaId, odontogramaData);

      Alert.alert(
        'Sucesso',
        'Odontograma atualizado com sucesso!',
        [
          {
            text: 'OK',
            onPress: () => {
      ...
      Alert.alert(
        'Erro',
        error.error || 'Erro ao atualizar odontograma. Tente novamente.',
        [{ text: 'OK' }]
      );
    } finally {
      setLoading(false);
      ...
      >
        ...
        >
          ...
          <View style={styles.header}>
            ...
            <Text style={styles.headerTitle}>Editar Odontograma</Text>
